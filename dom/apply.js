...
(function initApplyLayer() {
  ...
  function processTable(table) {
    ...
    for (var cc = 0; cc < cols.length; cc++) {
      // Column direction from its DATA cells' own reading direction (row 0 is the
      // header, skipped for the decision). A column that contains ANY Hebrew-
      // reading cell is a Hebrew column, so the WHOLE column -- header, and even
      // its English-technical cells -- right-aligns UNIFORMLY (what a reader
      // expects: "check | owner" both hug the right). Char-count majority was
      // wrong here: a column of Hebrew sentences peppered with English terms
      // ("Firestore", "deploy", "ALL-PASS") counts as majority-English yet reads
      // as Hebrew. A purely-English data column (no Hebrew cell) stays LTR (left).
      var rtlCells = 0, ltrCells = 0;
      for (var dr = 1; dr < grid.length; dr++) {
        var dcell = grid[dr][cc];
        if (!dcell) continue;
        var dcd = resolvedDir(dcell.textContent || '');
        if (dcd === 'rtl') rtlCells++; else if (dcd === 'ltr') ltrCells++;
      }
      var cdir = rtlCells > 0 ? 'rtl' : (ltrCells > 0 ? 'ltr' : null);
      for (var rr = 0; rr < grid.length; rr++) {
        ...
        // and uniformly per column, independent of the table's own dir.
        if (cdir === 'rtl') setAttr(cell, 'dir', 'rtl');
        else if (cdir === 'ltr') setAttr(cell, 'dir', 'ltr');
        else setAttr(cell, 'dir', 'auto');
      }
    }
    return true;
  }
